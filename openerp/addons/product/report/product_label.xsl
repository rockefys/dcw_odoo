...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
	...
	<xsl:template match="lots">
		<document>
			<template pageSize="(15cm,11cm)" allowSplitting="20" leftMargin="2.0cm" rightMargin="2.0cm" topMargin="2.0cm" bottomMargin="2.0cm" title="Address list" author="Generated by Odoo">
				<pageTemplate id="all">
					<frame id="all" x1="2cm" y1="0.1cm" width="19.0cm" height="10cm"/>
					<pageGraphics/>
	...
	<xsl:template match="lot-line" mode="story">
            <blockTable style="mytable" >
                <tr>
                    <td>
                        <para style="nospace" t="1" fontSize="20"><xsl:value-of select="code"/><xsl:text>             </xsl:text><xsl:value-of select="substring(product, 1, $pmaxChars)"/></para>
                    
                        <!-- <para style="nospace" t="1"><xsl:value-of select="substring(product, 1, $pmaxChars)"/><xsl:text>  </xsl:text><xsl:value-of select="currency"/></para> -->
                    </td>
                    <td>
                ...
                <tr>
                	<td>
                        <barCode code="code128" barWidth="2" barHeight="40"><xsl:value-of select="ean13" /></barCode> 
                    </td>
                    <td></td>
                </tr>
            </blockTable>
